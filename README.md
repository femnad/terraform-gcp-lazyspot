...
| <a name="input_firewall"></a> [firewall](#input\_firewall) | Firewall specification | <pre>object({<br>    other = optional(map(map(list(string))))<br>    self = optional(object({<br>      allow   = optional(map(list(string)))<br>      ip_mask = optional(number)<br>      ip_num  = optional(number)<br>      }), {<br>      allow = {<br>        icmp = []<br>        tcp  = ["22"]<br>      }<br>      }<br>    )<br>  })</pre> | `{}` | no |
